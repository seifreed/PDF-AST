use pdf_ast::security::{security_info_to_report, security_report_to_toml, SecurityAnalyzer};
use pdf_ast::types::{PdfDictionary, PdfName, PdfString, PdfValue};
use pdf_ast::{ast::NodeType, PdfDocument, PdfVersion};
use std::io::Cursor;

#[test]
fn security_report_serializes_to_toml() {
    ...
    let toml_str = security_report_to_toml(&report).expect("toml");
    let value: toml::Value = toml_str.parse().expect("parse");
    assert!(value.get("report_format_version").is_some());
}
