use pdf_ast::security::{security_info_to_report, security_report_to_yaml, SecurityAnalyzer};
use pdf_ast::types::{PdfDictionary, PdfName, PdfString, PdfValue};
use pdf_ast::{ast::NodeType, PdfDocument, PdfVersion};
use std::io::Cursor;

#[test]
fn security_report_serializes_to_yaml() {
    ...
    let yaml = security_report_to_yaml(&report).expect("yaml");
